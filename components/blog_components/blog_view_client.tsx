...
const BlogInnerContent = (props: { secc: string; title: string }) => {
  if (props.title.includes("otebook")) {
    let counter = 1;
    let modifiedHtmlString = props.secc.replace(/<code([^>]*)>/g, (_, p1) => {
...
const ExcalidrawFromDataDiv = (props: {
  secc: string;
  drawings: any;
  theme?: string;
}) => {
...
const BlogViewClient = (props: {
  ...
  if (editMode) {
    return (
      <div className="flex flex-col gap-4 w-full">
        ...
        <div className="flex flex-col gap-4">
          <div
            className="py-2 px-4 bg-zinc-800 text-zinc-200 cursor-pointer rounded"
            onClick={async () => {
          ...
          <div
            dangerouslySetInnerHTML={{ __html: htmlStringEditPreview }}
          ></div>
          <div
            className="py-2 px-4 bg-zinc-800 text-zinc-200 cursor-pointer rounded"
            onClick={async () => {
    ...
    return (
      <div>
        {session.data?.user?.email == props.authorEmail ? (
        ...
        <div>
          {props.htmlSectionsSeppedByFiles.map(
            (secc: string, curIndex: number) => {
              return (
                <div key={curIndex}>
                  {secc.slice(0, 19) == "<div data-filename=" ? (
                    <ExcalidrawFromDataDiv
                      secc={secc}
                      drawings={drawings}
                      theme={theme}
                    />
                    ...
                    <BlogInnerContent
                      secc={secc}
                      title={getTitleFromContent(props.markdownText)}
                    />
                  )}
                </div>
              );
